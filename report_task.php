...
$field = isset($_GET['field']) ? strval($_GET['field']) : "model";
$sort = isset($_GET['sort']) ? strval($_GET['sort']) : "ASC";
...
$sql = "
    ...
    ORDER BY s.contract_id, " . ($field) . " " . ($sort) . "
";
...
echo '<fieldset><input type="radio" id="asc" name="sort" value="ASC" checked/>  <label for="asc">ASC</label> <input type="radio" id="desc" name="sort" value="DESC" />  <label for="desc">DESC</label></fieldset>';
...
echo "<option value='s.quantity' selected='selected'>Количество</option>";
echo "<option value='m.price'>Цена</option>";
echo "<option value='model_name'>Название</option>";
echo "<option value='model'>Модель</option>";
echo "<option value='total_price'>Стоимость</option>";
echo "</select>";
